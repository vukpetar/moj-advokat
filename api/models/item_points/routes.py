...
async def create_item(item: ItemPointSchemas.ItemPointCreate, db: Session = Depends(get_db)):

    return ItemPointCrud.create_item_point(db=db, item=item)

@router.get("/", response_model=list[ItemPointSchemas.ItemPoint])
async def read_item_points(skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
